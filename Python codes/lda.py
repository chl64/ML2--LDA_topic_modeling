...
def LDA(A, B, K, alpha, gamma):
    ...
    r = 0    # Sanity check: at the end `r` should equal the total no of words in the train set.
    
    print('1st big loop (Initialisation).')
    for d in tqdm(range(D)):  # iterate over the documents
        z = np.zeros((W, K))  # unique word topic assignment counts for doc d
        
        words_in_doc_d = A[np.where(A[:, 0] == d+1), 1][0] - 1    # vocab IDs in doc d.
                                                                  # minus 1 converts them into python indices 
                                                                  # for convenience in access matrix elements.
        
        for w in words_in_doc_d:  # loop over the vocabs in doc d
                                  # in order to randomly assign every single word in the doc a topic k.
                                  # Then put each word into the counts for topic k (counts wrt vocab w --> beta, and doc d --> theta)
                    
            c = swd[w, d]  # counts for vocab w in doc d.
            
            for i in range(c):  # assign each occurrence of vocab w to a topic k at random
    ...
    for iter in tqdm(range(num_gibbs_iters)):
        print('2nd level in 2nd big loop.')
...
#                 print('a = \n', a)
    ...
    for d in unique_docs_in_b:  # loop over all documents in B
        ...
        num_gibbs_iters = 50
        for iters in tqdm(range(num_gibbs_iters)):
            for w in words_in_d:  # w are the words in doc d
                a = z[w, :].copy()  # number of times word w is assigned to each topic in doc d
